...
export const geminiProxy = onRequest({ timeoutSeconds: 300, memory: '512MiB', cors: true }, (req, res) => {
...
export const confirmPayment = onRequest({ cors: true }, (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
...
export const generateEvaluationReport = onRequest({ timeoutSeconds: 540, memory: '2GiB', concurrency: 10, cors: true }, (req: any, res: any) => {
    corsHandler(req, res, async () => {
    ...
    ## 5. 珥앺룊 諛??숈뒿 媛쒖꽑 ?쇰뱶諛?
            ...
            const config: any = { temperature: 0.1, topP: 0.1 };
            if (model.includes('pro')) config.thinkingConfig = { thinkingBudget: 16000 };
            else config.thinkingConfig = { thinkingBudget: 4000 };

            console.log(`[EvalReport] Starting Gemini evaluation generation. Model: ${model}. Ref parts: ${referenceParts?.length}, Student parts: ${studentParts?.length}`);
            // Step 1: Grading
            const gradingResponse = await ai.models.generateContent({
                ...
                contents: {
                    parts: [
                        ...
                        ...(referenceParts || []).map((p: any) => ({ inlineData: p })),
                        { text: "\n\n### STUDENT SOLUTION (TO BE GRADED) ###\n" },
                        ...studentParts.map((p: any) => ({ inlineData: p })),
                    ],
                },
                config,
            });
...
export const processAssetCsvTrigger = onDocumentCreated({
    ...
    try {
        ...
        for (const rawRow of records) {
            ...
            const tags = [];
            if (university) tags.push(university);
            if (year) tags.push(year);
            if (category) tags.push(category);

            // ?대떦 ?됱씠 ?꾩쟾??鍮꾩뼱?덈뒗 ?곕젅湲?媛믪씠硫??ㅽ궢
            if (!university && !year && !questionLink) continue;
...
export const chatTutor = onRequest({ timeoutSeconds: 60, memory: '256MiB', cors: true }, (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
...
export const migrateOldAssets = onRequest({ timeoutSeconds: 540, memory: '1GiB' }, async (req, res) => {
    try {
...
export const askTutorAi = onRequest({ timeoutSeconds: 300, memory: '512MiB', cors: true }, (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
...
export const analyzeProblemImage = onRequest({ timeoutSeconds: 60, memory: '512MiB', cors: true }, (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
...
export const ragTutorImage = onRequest({ timeoutSeconds: 120, memory: '1GiB', cors: true }, (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
            ...
            try {
                ...
                if (searchResponse.ok) {
                    ...
                    if (searchData.results) {
                        retrievedContext = searchData.results.map((r: any) => {
                            const snippets = r.document?.derivedStructData?.snippets?.map((s:any) => s.snippet).join(" ") || "";
                            const extracts = r.document?.derivedStructData?.extractive_answers?.map((e:any) => e.content).join(" ") || "";
...
export const generateSimilarProblems = onRequest({ timeoutSeconds: 300, memory: '1GiB', cors: true }, (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
...
export const autoTagProblem = onRequest({ timeoutSeconds: 60, memory: '512MiB', cors: true }, (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
        ...
        try {
            ...
            try {
                ...
                if (searchResponse.ok) {
                    ...
                    if (searchData.results) {
                        retrievedContext = searchData.results.map((r: any) => {
                            const title = r.document?.derivedStructData?.title || r.document?.id || "";
                            const snippet = r.document?.derivedStructData?.snippets?.map((s:any) => s.snippet).join(" ") || "";
                            return "[출처: " + title + "]\n" + snippet;
                        }).join("\n\n");
                    }
                }
