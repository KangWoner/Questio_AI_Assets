...
import { auth, getPurchasedAssets, getAssetsByIds, AssetData } from '../firebase';
  ...
  return (
    <div className="bg-stone-900 border border-stone-800 rounded-[2.5rem] p-8 flex flex-col h-full group transition-all hover:bg-stone-800/40">
      ...
      <div className="flex-grow space-y-3 overflow-y-auto max-h-56 min-h-[140px] scrollbar-thin pr-3">
        {files.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-700/50 rounded-[2rem] py-10 opacity-30">
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">Questio Assets</span>
          </div>
        ) : (
          files.map((f, i) => (
            <div key={i} className="flex items-center justify-between bg-stone-950/60 border border-stone-800 px-5 py-4 rounded-2xl group/item hover:border-stone-600 transition-all">
              <div
                className="flex items-center min-w-0 flex-1 mr-4 cursor-pointer hover:opacity-80 transition-opacity"
                onClick={(e) => {
                  e.stopPropagation();
                  if (f.url) {
                    // Open in new tab for browsing and native download
              ...
              >
                ...
                <span className={`text-xs truncate ${f.url ? 'text-sky-400 font-black underline underline-offset-2' : 'text-stone-400 font-bold underline underline-offset-2'}`}>{f.name}</span>
              </div>
              <button onClick={() => onFilesChange(files.filter((_, idx) => idx !== i))} title="파일 삭제" aria-label="파일 삭제" className="text-stone-700 hover:text-red-500 transition-colors p-1">
...
export const InputForm: React.FC<InputFormProps> = ({ formData, onFormChange, onGenerate, onSearchCriteria, onGoToLibrary, isLoading, isSearching }) => {
  ...
  const availableRecords = useMemo(() => {
    ...
    try {
      ...
      const withIndex = sorted.map(r => {
        const key = `${r.university}-${r.year}-${r.problemType}`;
        counts[key] = (counts[key] || 0) + 1;
        return { ...r, _index: counts[key] };
      });

      return withIndex.map(r => {
        const key = `${r.university}-${r.year}-${r.problemType}`;
        const total = counts[key];
        const displayType = total > 1 ? `${r.problemType} ${String.fromCharCode(64 + r._index)}` : r.problemType;
  ...
  const processCsvText = (text: string, fileName: string) => {
    ...
    if (errors.length > 0) {
      ...
      setLastSyncedFileName(fileName);
  ...
  return (
    <div className="glass-panel p-12 rounded-[5rem] shadow-[0_0_40px_rgba(0,0,0,0.5)] space-y-12 relative">
      <div className="flex bg-slate-900/50 p-2 rounded-[2rem] border border-slate-700/50 shadow-inner backdrop-blur-sm">
        <button
          onClick={() => { onFormChange({ category: 'essay' }); setLastSyncedFileName(''); }}
          className={`flex-1 py-4 rounded-[1.5rem] text-[11px] font-black uppercase tracking-widest transition-all ${formData.category === 'essay' ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-[0_0_15px_rgba(59,130,246,0.6)]' : 'text-slate-500 hover:text-slate-300'}`}
        >
          Essay Logic (수리논술)
        </button>
        <button
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase italic drop-shadow-md">Config</h2>
          <p className="text-[10px] text-cyan-400 font-black tracking-widest uppercase mt-1">GCS Sync: {activeBucket}</p>
        </div>
        <div className="flex items-center space-x-4">
          <select
            ...
            className="bg-slate-900/60 text-[10px] font-black text-slate-300 border border-slate-700/50 px-6 py-3 rounded-2xl outline-none cursor-pointer hover:text-cyan-400 transition-all shadow-inner uppercase tracking-widest backdrop-blur-md"
          >
            {AVAILABLE_MODELS.map(m => (
              <option key={m} value={m} className="bg-slate-900">{modelLabels[m]}</option>
            ))}
          </select>
          <button onClick={() => setIsAdminOpen(!isAdminOpen)} title="관리자 설정 오픈" aria-label="관리자 설정 오픈" className={`p-3 border rounded-2xl transition-all shadow-inner ${isAdminOpen ? 'bg-sky-500/20 border-sky-500/50 text-sky-400' : 'bg-stone-950 border-stone-800 text-stone-600 hover:text-sky-500'}`}>
      ...
      {isAdminOpen && (
        <div className="p-8 bg-sky-500/5 border border-sky-500/10 rounded-[2.5rem] space-y-6 animate-in fade-in zoom-in duration-300">
          ...
          <div className="flex flex-col space-y-4">
            <div className="flex items-center space-x-4">
              <label className="cursor-pointer bg-slate-800 hover:bg-slate-700 text-stone-300 px-6 py-3 rounded-2xl text-[11px] font-black transition-all shadow-md uppercase tracking-widest shrink-0 border border-slate-700">
                내 PC
                <input type="file" accept=".csv" onChange={handleCsvUpload} className="sr-only" />
              </label>
              <button 
                onClick={handleOpenLibrary}
                className="cursor-pointer bg-sky-500 hover:bg-sky-400 text-white px-6 py-3 rounded-2xl text-[11px] font-black transition-all shadow-xl uppercase tracking-widest shrink-0"
              >
                내 보관함 (Asset Load)
              </button>
            </div>
            <div className="flex flex-col overflow-hidden">
              <span className="text-[11px] text-stone-400 font-bold truncate">
                {lastSyncedFileName ? `✅ 동기화: ${lastSyncedFileName}` : '선택된 에셋 없음'}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {availableRecords.map((r, i) => (
                <button
                  ...
                  className={`w-full text-left px-8 py-6 rounded-[2.5rem] transition-all border-2 flex flex-col justify-center group ${formData.problemType === r.problemType
                ...
                >
                  <div className="flex justify-between items-start w-full">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs font-black break-words leading-tight pr-2">{(r as any).displayType || r.problemType}</span>
                    </div>
        ...
        <div className="flex justify-between items-center px-2">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="px-4 py-1.5 bg-sky-500/10 border border-sky-500/20 rounded-xl text-[9px] font-black text-sky-500 hover:bg-sky-500 hover:text-white transition-all uppercase disabled:opacity-30"
            >
              Manual Discovery
            </button>
            {isSearching && (
      ...
      <div className="border-t border-slate-700/50 pt-12">
        <div className="flex justify-between items-center mb-10">
          <h3 className="text-xl font-black text-slate-100 uppercase tracking-tighter drop-shadow-sm">Candidate Registry</h3>
          <button onClick={() => onFormChange({ students: [...formData.students, { id: crypto.randomUUID(), name: '', email: '', solutionFiles: [], reportTemplate: '' }] })} className="bg-white/5 hover:bg-white/10 border border-white/10 px-8 py-3 rounded-2xl text-[11px] font-black text-white transition-all flex items-center shadow-lg uppercase backdrop-blur-sm">
            <PlusCircleIcon className="w-5 h-5 mr-3 text-cyan-400" /> Enroll New
          </button>
        </div>
        ...
        <div className="space-y-5 max-h-[25rem] overflow-y-auto pr-4 scrollbar-thin">
          {formData.students.length === 0 ? (
            ...
            formData.students.map((s) => (
              <div key={s.id} className="p-8 bg-slate-900/60 border border-slate-700/50 rounded-[3rem] relative group hover:border-cyan-500/50 transition-all shadow-[0_4px_20px_rgba(0,0,0,0.3)] backdrop-blur-md">
                <button onClick={() => onFormChange({ students: formData.students.filter(std => std.id !== s.id) })} title="학생 삭제" aria-label="학생 삭제" className="absolute top-8 right-8 text-slate-600 hover:text-red-400 transition-colors p-1">
                  <TrashIcon className="w-6 h-6" />
                </button>
                <div className="grid grid-cols-2 gap-6 mb-6">
                  <input value={s.name} onChange={(e) => onFormChange({ students: formData.students.map(item => item.id === s.id ? { ...item, name: e.target.value } : item) })} placeholder="성함" className="bg-slate-900/40 px-8 py-4 rounded-2xl border border-slate-700/50 text-xs text-slate-200 font-bold outline-none focus:border-cyan-400/50 focus:shadow-[0_0_15px_rgba(34,211,238,0.2)] transition-all" />
                  <input value={s.email} onChange={(e) => onFormChange({ students: formData.students.map(item => item.id === s.id ? { ...item, email: e.target.value } : item) })} placeholder="이메일" className="bg-slate-900/40 px-8 py-4 rounded-2xl border border-slate-700/50 text-xs text-slate-200 font-bold outline-none focus:border-cyan-400/50 focus:shadow-[0_0_15px_rgba(34,211,238,0.2)] transition-all" />
                </div>
                <label className="flex items-center justify-center w-full px-8 py-5 border-2 border-dashed border-slate-700/50 rounded-2xl cursor-pointer hover:border-cyan-400/50 hover:bg-cyan-500/10 transition-all group/upload">
                  <span className="text-[10px] font-black text-slate-400 group-hover/upload:text-cyan-400 uppercase tracking-widest transition-colors">
      ...
      <button
        ...
        className={`w-full py-8 text-white rounded-[3.5rem] font-black text-2xl shadow-2xl transform transition active:scale-[0.98] disabled:opacity-20 disabled:grayscale flex items-center justify-center uppercase italic ${formData.category === 'essay' ? 'bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 shadow-cyan-900/40' : 'bg-gradient-to-r from-fuchsia-600 to-purple-600 hover:from-fuchsia-500 hover:to-purple-500 shadow-fuchsia-900/40'}`}
      >
        {isLoading ? (
